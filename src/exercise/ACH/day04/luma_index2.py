...
from llama_index.core import OpenAI, Settings
Settings.llm = OpenAI(model="gpt-3.5-turbo")

# 페이지 제목 설정
st.title("간단한 LlamaIndex 테스트")
...
# 텍스트 입력
text = st.text_area("분석할 텍스트를 입력하세요:", "LlamaIndex는 대규모 언어 모델을 위한 데이터 프레임워크입니다.")
...
if st.button("인덱스 생성"):
    ...
    response = query_engine.query("이 텍스트는 무엇에 관한 것인가요?")
...
# 파일 업로드 기능
uploaded_file = st.file_uploader("문서를 업로드하세요", type=["pdf", "txt", "docx"])
...
def get_file_list():
    try:
        ...
        st.error(f"파일 목록 가져오기 중 오류 발생: {e}")
        return []
...
if uploaded_file:
    ...
    if os.path.exists(os.path.join(file_dir, "index")):
        ...
        if text:
            ...
            st.success("인덱스가 생성되었습니다!")
        else:
            st.error("파일 내용을 추출할 수 없습니다.")
