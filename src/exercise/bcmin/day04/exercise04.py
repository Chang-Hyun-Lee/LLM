...
def get_camping_info_context(keyword):
    ...
    ServiceKey = f"Qc1EZeeC07yCDsHqnRj1aSz%2FafKhQGm0m7v5m2jsH3WlKZuWZaPCfLrBzQuMqWTxgFeoJdXvneFgfioVxhK%2BjQ%3D%3D"
...
def get_restaurant_info(place, kind):
    ...
    kakao_api_key = "fec0d451a44400e161bb68d466933d9d" #os.getenv("KAKAO_API_KEY")
...
def init_agent():
    ...
    prompt.messages[0] = SystemMessagePromptTemplate.from_template("당신은 캠핑과 레스토랑 추천 전문가입니다. " \
            "주어진 정보를 보고 고객이 원하는 캠핑장이나 레스토랑을 검색해서 답변해야 합니다. " \
            "답변할 수 없으면 추천할 캠핑장이나 레스토랑이 없다고 답변하세요.")

    tools = [get_camping_info_context, get_restaurant_info]
...
def main():
    st.title("Multi-turn Chatbot with Streamlit and OpenAI(+ 캠핑 or 레스토랑)")
