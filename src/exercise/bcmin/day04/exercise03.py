...
@tool(description="ambigus company name")
def get_company_name():
    tickers = stock.get_market_ticker_list(market="ALL")
    names = list()
    for ticker in tickers:
        name = stock.get_market_ticker_name(ticker)
        names.append({"ticker": ticker, "company_name": name})

    return (names)

@tool(description="Get market OHLCV data")
...
def init_agent():
    ...
    "단 주식종목 이름이 정확하지 않을 경우 제일 근접한 이름의 회사에 대해 알려주세요.")

    tools = [get_market_ohlcv, get_company_name]
...
def main():
    st.title("Multi-turn Chatbot with Streamlit and OpenAI(+ Stock)_애매한 이름포함")
