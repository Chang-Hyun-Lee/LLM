...
def get_market_ohlcv(start_date, end_date, ticker):
    """Return prices within given dates for ticker stock. start_date and end_date shoule be 'YYYYMMDD' format. You must get stock price by only this tool."""
...
def init_agent():
    ...
    prompt.messages[0] = SystemMessagePromptTemplate.from_template("당신은 친절한 어시스턴트입니다. 주어진 도구들을 사용하여 사용자의 요청에 친절하게 답변하세요. 주식정보의 종목코드(티커)는 6자리 숫자입니다. 종목코드를 모르면 검색해서 찾은 후 답변하세요. 답변할 수 없으면 답변할 수 없다고 대답하세요.")

    tools = [get_camping_info_context, get_restaurant_info, get_market_ohlcv, get_ticker_info_by_search]
